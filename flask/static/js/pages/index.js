...
function productCard(el, carousel = false) {
  ...
  if (el.hasOwnProperty("prix")) {
    ...
    const html = `
                    ...
                    ${promo}
                    <div class="row">
                        <span class="card-title grey-text text-darken-4 col s12 center">${
                          el.nomp
                        }</span>
                    </div>
                    <hr>
  ...
  cat.then(
    (value) => {
      ...
      const checkboxes = Array.from(
        document.getElementsByClassName("filled-in")
      );
  ...
  products.then(
    function (value) {
      ...
      const filteredProducts = value.filter(function (el) {
        ...
        if (filter.materiaux.length > 0)
          t = t && filter.materiaux.includes(el.materiaux);
        //filtrage du prix
