...
function populateNavBarIcons() {
  ...
  switch (page) {
    ...
    case "":
    case "index":
      document.getElementById("searchLI").appendChild(
        link(function () {
          openSlide("searchSlide");
        }, "search")
      );
      document.getElementById("profileLI").appendChild(
        link(function () {
          openSlide();
        }, "account_circle")
      );
...
function HTMLParser(html, mode = "text/html") {
  if (mode === "text/html") {
     ...
     */
    return new DOMParser().parseFromString(html, mode).firstChild.lastChild
      .firstChild;
  } else {
