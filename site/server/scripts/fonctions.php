...
function connexionToApp($login, $pass)
{
    ...
    $res = callAuthApi("/auth", $credits, "POST");
    var_dump($res);

    //Si la réponse est vraie alors on créer une session et on écrit dans les log la bonne connexion puis on renvoit Vrai
    if ($res["status"] == 200 && $res["response"]["status_code"] == 200) {
        $_SESSION["login"] = $login;
        $_SESSION["statut"] = $res["response"]["statut"];
        writeLog("Connexion réussie pour l'utilisateur : $login [statut :" . $res["response"]["statut"] . "]");
        return true;
    }
...
function addProduct($nom, $prix, $promo, $type, $mat, $path)
{
    ...
    $res = callDataApi("/produits", $produit, "POST");
